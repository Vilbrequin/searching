...
search_lib = ctypes.CDLL('../functions/jump_search.o')

# Define the argument types and return type of the C function
search_lib.jump_search.argtypes = [ctypes.POINTER(ctypes.c_int), ctypes.c_int, ctypes.c_int]
search_lib.jump_search.restype = ctypes.c_int

class TestJumpSearchFunction(unittest.TestCase):

    def test_element_found(self):
        arr = (ctypes.c_int * 5)(1, 2, 3, 4, 5)
        size = len(arr)
        element = 3
        result = search_lib.jump_search(arr, size, element)
        self.assertEqual(result, 2)

    def test_element_not_found(self):
        arr = (ctypes.c_int * 8)(11, 32, 43, 54, 58, 88, 98, 122)
        size = len(arr)
        element = 6
        result = search_lib.jump_search(arr, size, element)
    ...
    def test_large_array(self):
        ...
        size = len(arr)
        element = 750
        result = search_lib.jump_search(arr_c, size, element)
